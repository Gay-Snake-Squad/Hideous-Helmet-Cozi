	...
	void DrawCommonStuff(bool usemughud){
		...
		if(cplayer.readyweapon&&cplayer.readyweapon!=WP_NOCHANGE)
			...
			if(helmet||!hh_hidecompass.getbool()){
				...
				string postxt=string.format("%i",hpl.angle);
				screen.DrawText(NewSmallFont,
					font.CR_OLIVE,
				...
				//console.printf("STIM "..HDStim);
				//This is the grid the HUD uses, and honestly I've never seen use for it so I'm not gonna use it.
